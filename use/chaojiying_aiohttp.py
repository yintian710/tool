...
from tool.asybase import AsyncBase


class Chaojiying_Client(AsyncBase):
    def __init__(self, username, password, soft_id, nums=None):
        super().__init__('cjy', nums=nums)
    ...
    async def PostPic(self, img, codetype=9101):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post('http://upload.chaojiying.net/Upload/Processing.php', data=params, files=files,
    ...
    async def ReportError(self):
        ...
        params.update(self.base_params)
        async with aiohttp.ClientSession() as session:
            async with session.post('http://upload.chaojiying.net/Upload/Processing.php', data=params,
                                    headers=self.headers) as resp:
                res = await resp.json()
        print('执行报错')
        return res
